...
def nlvr2_create_dictionary_glove(nlvr2_q='data/NLVR2-Questions', glove='data/GloVe/glove.6B.300d.txt',
                                  cache='data/NLVR2-Cache'):
    """ Create Dictionary from NLVR2 Question Files, and Initialize GloVe Embeddings from File """
    ...
    questions = ['train.json', 'dev.json', 'test1.json']

    # Iterate through Question/Statement in Question Files and update Vocabulary
    print('\t[*] Creating Dictionary from NLVR-2 Questions...')
    for qfile in questions:
        qpath = os.path.join(nlvr2_q, qfile)
        ...
        # Iterate through examples and assemble dictionary
        for ex in examples:
            dictionary.tokenize(ex['sentence'], add_word=True)
